...
from perm_equivariant_seq2seq.g_utils import g_transform_data, g_inverse_transform_data, g_inverse_transform_data_prob
...
class MultiEquiSCANModel(nn.Module):
    ...
    def forward(self, input_tensor, target_tensor, use_teacher_forcing=False):
        ...
        for i in range(self.num_G):
            ...
            in_G_inv = [self.in_G[j].cinv for j in inds]

            out_G = [self.out_G[j].g for j in inds]
            out_G_inv = [self.out_G[j].cinv for j in inds]

            inversed_input_tensor = g_inverse_transform_data(input_tensor, in_G_inv, device)
            inversed_target_tensor = g_inverse_transform_data(target_tensor, out_G_inv, device)

            transformed_input_tensor = g_transform_data(inversed_input_tensor, self.in_G[i].g, device)
            transformed_target_tensor = g_transform_data(inversed_target_tensor, self.out_G[i].g, device)
            ...
            outputs = g_inverse_transform_data_prob(transformed_outputs, G=self.out_G[i].g)
            model_output = torch.mean(outputs, dim=0, keepdim=False)[None, :, :]

            for G in out_G:
                model_output = torch.mean(g_inverse_transform_data_prob(model_output.repeat(len(G), 1, 1), G=G), dim=0)
