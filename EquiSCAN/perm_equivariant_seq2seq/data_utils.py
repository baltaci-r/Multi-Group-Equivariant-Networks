...
def get_scan_split(split=None, expanded=False):
    assert split in ['simple', 'jump', 'turn_left', 'turn_up', 'turn_up_jump_turn_left',
                     'length_generalization', 'around_right', 'opposite_right'], \
    ...
    if expanded == False:
        ...
        # Add length generalization split
            ...
            train_path = os.path.join(dir_path, 'tasks_train_length.txt')
            test_path = os.path.join(dir_path, 'tasks_test_length.txt')
    else:

        # Add jump generalization split
        dir_path = os.path.join(DATA_DIR, 'expanded_dataset', 'add_prim_split')
        train_path = os.path.join(dir_path, f'tasks_train_addprim.txt')
        test_path = os.path.join(dir_path, f'tasks_test_addprim_{split}.txt')
