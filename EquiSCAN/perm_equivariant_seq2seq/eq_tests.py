...
def get_group_product(*groups):
    for G in groups:
        ...
        for tup in product(*G):
            g = {}
            for k, v in enumerate(zip(*[d.values() for d in tup])):
                a = [k == i for i in v]
                if all(a):
                    g.update({k: v[0]})
                else:
                    assert sum(a) == len(a) - 1
                    g.update({k: v[a.index(False)]})
            P.append(g)
        yield P
...
def equizerotest(input_tensor,
    ...
    plt.imshow(transformed_model_output.detach().numpy())
    plt.show()
    plt.imshow(model_transformed_output.detach().numpy())
    plt.show()
...
def main(args):
    ...
    if args.layer_type in ['RNN', 'GRU', 'LSTM']:
        ...
        for g in args.equivariance:
            ...
            in_g = cyclic_group_generator(vocab_size=equivariant_commands.n_words,
                                          eq_indices=equivariant_commands_indices)
            ...
            out_g = cyclic_group_generator(vocab_size=equivariant_actions.n_words,
                                           eq_indices=equivariant_actions_indices)
    ...
    in_G, out_G = get_group_product(in_Gs, out_Gs)
    equivariant_actions_indices = all_equivariant_actions_indices
