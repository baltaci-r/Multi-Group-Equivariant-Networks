...
from perm_equivariant_seq2seq.g_utils import cyclic_group_generator, cyclic_group, cyclic_group_inv, g_transform_data, g_inv_transform_prob_data, g_transform_prob_data


device = torch.device("cuda" if torch.cuda.is_available() else "cpu")


def multiequitest(input_tensor,
          target_tensor,
          model,
          in_G,
          out_G):
    ...
    model_output = model(input_tensor=input_tensor, target_tensor=target_tensor)
...
def main(args):
    ...
    if 'direction-ud' in args.equivariance: assert args.expanded
    ...
    if args.layer_type in ['RNN', 'GRU', 'LSTM']:
        ...
        in_Gs,  out_Gs, all_equivariant_actions_indices = [], [], []
        # get equivariant words
        for g in args.equivariance:
            if g == 'verb':
                ...
                in_equivariant_words = ['up', 'down']
                out_equivariant_words = ['TURN_UP', 'TURN_DOWN']
            else:
                in_equivariant_words = []
                out_equivariant_words = []

            equivariant_commands_indices, equivariant_actions_indices = \
                [equivariant_commands.word2index[word] for word in in_equivariant_words], \
                [equivariant_actions.word2index[word] for word in out_equivariant_words]
            ...
            in_G = cyclic_group(g=in_g, group_size=len(equivariant_commands_indices), vocab_size=equivariant_commands.n_words)
            in_G_inv = cyclic_group_inv(indices=equivariant_commands_indices, vocab_size=equivariant_commands.n_words)
            ...
            out_G = cyclic_group(g=out_g, group_size=len(equivariant_actions_indices), vocab_size=equivariant_actions.n_words)
            out_G_inv = cyclic_group_inv(indices=equivariant_actions_indices, vocab_size=equivariant_actions.n_words)

            in_Gs.append((in_G, in_G_inv))
            out_Gs.append((out_G, out_G_inv))
            all_equivariant_actions_indices.append(equivariant_actions_indices)
    else:
    ...
    multiequitune_model = MultiEquiSCANModel(pre_model=model, in_G=in_Gs, out_G=out_Gs, vocab_size=equivariant_actions.n_words,
                                   eq_word_indices=all_equivariant_actions_indices,
    ...
    for iteration in tqdm(range(1, args.n_iters + 1)):
        ...
        for in_G, out_G in zip(in_Gs, out_Gs):
            multiequitest(input_tensor=iteration_input,
                             target_tensor=iteration_output,
                             model=multiequitune_model,
                             in_G=in_G[0],
                             out_G=out_G[0])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--split',
                        ...
                        choices=[None, 'simple', 'jump', 'turn_left', 'turn_up', 'turn_up_jump_turn_left', 'around_right', '', ''],
    ...
    parser.add_argument('--expanded',
                        action='store_true',
                        help="Boolean to use expanded dataset with 3rd dimension (up, down)")
